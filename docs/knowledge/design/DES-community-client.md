...
评论首屏和切排序递增读取代次；旧首屏或旧分页的成功、失败均不得覆盖当前排序及页号。
...
资料列表刷新接管读取时同时结束旧分页 loading；即使刷新失败，也保留原列表与游标并恢复分页重试。
...
事件在异步初始化前固定 ownerIdentity 并持久化：认证事件绑定精确 userId，同账号重新登录可继续
发送原事件；匿名事件绑定本地会话 revision，登录后不改签为认证行为。flush 只选择当前身份的条目，
其它账号积压不阻塞当前账号，发送前再次校验 owner 并绑定请求 revision。旧队列缺少 owner 的记录
无法可信补造归属，保留为不可发送项；恢复进程或换号都不将其归给当前用户。
