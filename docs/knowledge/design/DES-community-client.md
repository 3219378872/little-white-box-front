...
编辑器在路由 postId 变化时清理旧表单、上传缓存和写入身份，再加载新帖子。加载、上传与提交共享
编辑器代次；旧请求不能填充、关闭或清理新编辑器，提交始终使用开始时冻结的 postId 和 revision。
...
首屏刷新失败与分页失败分别记录，重试精确重放失败页；保留旧评论或已读到末页不改变失败页身份。
楼中楼另按父评论记录请求代次，收起、重新展开和列表刷新均使旧回复请求失效。发送完成只清理仍与
提交时相同的草稿，期间输入的新草稿保持不变。
回复第一页失败时尚无成功页号，重试仍取第一页；发送成功清空线程缓存时同时收起线程并清除页号与
loading，下次展开重新加载第一页，避免跳页遗漏回复。
...
同帖卡片与详情共享乐观关系，但各自以服务器关系快照计算当前用户贡献差；关系已一致时直接显示
服务器计数，不叠加永久增量。失败恢复操作前的覆盖值，点赞失败不回滚并行成功的收藏。
资料列表刷新接管读取时同时结束旧分页 loading；即使刷新失败，也保留原列表与游标并恢复分页重试。
资料页从现有用户资料接口读取当前访问者的 `isFollowing`；局部乐观覆盖只属于当前会话和目标用户，
失败恢复先前值，离开再进入时重新读取关系。Mock 使用相同的访问者维度，不将关系写入公共用户资料。
