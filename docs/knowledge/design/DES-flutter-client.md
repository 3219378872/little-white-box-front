...
个人资料的帖子列表与收藏列表共用 `userPostsProvider(UserPostsKey)`。收藏列表在每次成为当前
tab 时重新请求第一页；从其它路由返回且仍停留在收藏 tab 时同样重新请求。请求代次丢弃陈旧的
首屏响应，避免会话内旧缓存挡住刚收藏的帖子。帖子 tab 仍按首次加载与下拉刷新更新。
